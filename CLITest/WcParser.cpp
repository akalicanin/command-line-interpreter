...
CommandData* WcParser::parse(std::string& input, bool skipInput, bool skipOutput) {
    ...
    if (optName != "-w" && optName != "-c") {
        std::cerr << "Error! Invalid opt argument for command: " << cmdName << '!' << std::endl;
        return nullptr;
    }
    ...
    //if (!stream.eof()) {
    //    std::cerr << "Error! Invalid additional arguments for command: " << cmdName << '!' << std::endl;
    //    return nullptr;
    //}
